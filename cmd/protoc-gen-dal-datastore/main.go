...
func main() {
	...
	dalFilenameSuffix := flags.String("dal_filename_suffix", "_dal", "Suffix for DAL helper filename (e.g., '_dal' -> 'user_datastore_dal.go')")
	dalFilenamePrefix := flags.String("dal_filename_prefix", "", "Prefix for DAL helper filename (e.g., 'dal_' -> 'dal_user_datastore.go')")
	dalOutputDir := flags.String("dal_output_dir", "", "Subdirectory for DAL files relative to main output (e.g., 'dal' -> 'gen/datastore/dal/')")
	...
	protogen.Options{
		...
		var dalResult *datastore.GenerateResult
		if *generateDAL {
			dalResult, err = datastore.GenerateDALHelpers(messages, &datastore.DALOptions{

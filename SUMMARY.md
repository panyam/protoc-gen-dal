...
| Oneof field handling | Oneof declarations create individual fields (not a single field with the oneof name). Example: `oneof move_type { MoveUnitAction move_unit = 4; }` creates a field named "move_unit" (not "move_type"). Field merging by name means oneof members get pulled into target unless handled. Two approaches: (1) Define target mappings for each oneof member type (e.g., MoveUnitActionGORM with source annotation) - recommended for complex types. (2) Skip individual oneof fields using `skip_field` annotation on each member and store as google.protobuf.Any instead. Future enhancement: message-level `skip_fields: ["field1", "field2"]` annotation for cleaner syntax. |

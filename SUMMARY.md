...
| Converter registry for nested types | Track available converters in a registry. Generate nested converter calls (via `mustConvert` helpers) only when converter exists. Skip fields requiring manual handling. Enables recursive conversions without manual wiring. |
